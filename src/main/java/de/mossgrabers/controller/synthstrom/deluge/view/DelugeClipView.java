 ...
 */
public class DelugeClipView extends AbstractNoteSequencerView<DelugeControlSurface, DelugeConfiguration>
    ...
    public void updateNoteMapping() {
        // Skip super.updateNoteMapping () (which schedules an empty table update)
        // to avoid race conditions or flickering.
        this.updateScale();
    }
    ...
    protected void handleSequencerArea(final int index, final int x, final int y, final int velocity) {
        ...
        if (this.noteEditPosition != null) {
            ...
            // Explicitly calculate step using local page offset logic to ensure it matches
            // drawGrid
            final int page = this.model.getCursorClip().getEditPage();
            final int offset = page * this.clipCols;
            final int step = offset + x;
            final int note = this.keyManager.map(y);
            this.getClip().toggleStep(new NotePosition(this.configuration.getMidiEditChannel(), step, note),
    ...
    /** {@inheritDoc} */
    @Override
    protected boolean handleSequencerAreaButtonCombinations(final INoteClip clip, final NotePosition notePosition,
            final int row, final int velocity) {
    ...
    public void drawGrid() {
        ...
        final double loopEnd = loopStart + clip.getLoopLength();
        final double stepLength = Resolution.getValueAt(this.getResolutionIndex());

        for (int x = 0; x < this.clipCols; x++) {
            final int currentStep = offset + x;
            final double stepStart = currentStep * stepLength;
            final boolean isInLoop = stepStart >= loopStart && stepStart < loopEnd;

            // Gray out entire column if outside loop
            if (!isInLoop) {
                for (int y = 0; y < this.numSequencerRows; y++)
                    gridPad.lightEx(x, 7 - y, DelugeColorManager.DELUGE_COLOR_GRAY); // Use gray for out of loop
                continue;
            }

            if (x >= clip.getNumSteps()) {
                // Inside loop but beyond current clip addressable range in this view (e.g. end
                // of page)
                for (int y = 0; y < this.numSequencerRows; y++)
            ...
            // Use relative step (x) for accessing clip data via CursorClip (which is paged)
            ...
            for (int y = 0; y < this.numSequencerRows; y++) {
                ...
                gridPad.lightEx(x, 7 - y,
                        this.getStepColor(stepInfo, x == hiStep, notePosition.getChannel(), x, y, map, editNotes));
            }
        }
         ...
         * final int loopEndPad = (int) Math.ceil ((loopStart + clip.getLoopLength ()) /
         * lengthOfOnePad);
         * final int currentPage = step / this.clipCols;
         * for (int pad = 0; pad < this.clipCols; pad++)
         * gridPad.lightEx (pad, 0, this.getPageColor (loopStartPad, loopEndPad,
         * currentPage, clip.getEditPage (), pad));
         */
    ...
    /**
     * Toggle between straight and triplet resolution.
     */
    public void toggleTriplets() {
        final int currentIndex = this.getResolutionIndex();
        final boolean isTriplet = currentIndex % 2 != 0;
        final int newIndex = isTriplet ? currentIndex - 1 : Math.min(7, currentIndex + 1);
        this.setResolutionIndex(newIndex);
        this.surface.getDisplay().notify(Resolution.getNameAt(newIndex));
    }
    ...
    /** {@inheritDoc} */
    @Override
    protected String getStepColor(final IStepInfo stepInfo, final boolean isHighlighted, final int channel, final int x,
            final int y, final int note, final List<NotePosition> editNotes) {
        if (isHighlighted)
