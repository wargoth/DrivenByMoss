...
public class DelugeControllerSetup extends AbstractControllerSetup<DelugeControlSurface, DelugeConfiguration> {
     ...
     */
    public DelugeControllerSetup(final IHost host, final ISetupFactory factory, final ISettingsUI globalSettings,
            final ISettingsUI documentSettings) {
    ...
    protected void createModel() {
        ...
        ms.setNumMarkers(8);
        ms.setNumParams(8);
        this.model = this.factory.createModel(this.configuration, this.colorManager, this.valueChanger, this.scales,
                ms);
    }
    ...
    protected void createSurface() {
        ...
        final DelugeControlSurface surface = new DelugeControlSurface(this.host, this.colorManager, this.configuration,
                output, input);
    ...
    public void flush() {
        ...
        if (activeView instanceof DelugeClipView) {
            final DelugeClipView clipView = (DelugeClipView) activeView;
            final boolean isTriplet = clipView.getResolutionIndex() % 2 != 0;
            surface.setButtonLED(DelugeControlSurface.BUTTON_TRIPLETS, isTriplet);
        }
    }
    ...
    private void handleHorizontalEncoder(final int delta) {
        ...
        if (this.xEncoderPressed) {
            if (activeView instanceof DelugeClipView) {
                ...
                // Step by 2 to jump between straight resolutions (0, 2, 4, 6)
                final int step = 2;
                final int newRes = delta > 0 ? Math.min(7, currentRes + step) : Math.max(0, currentRes - step);
        ...
        if (delta > 0) {
            ...
            if (activeView instanceof DelugeClipView) {
                ...
                final double stepLength = Resolution.getValueAt(clipView.getResolutionIndex());
                // Check if next page starts within the loop
                final double nextPageStartBeat = (clip.getEditPage() + 1) * 16 * stepLength;
                if (nextPageStartBeat >= clip.getLoopStart() + clip.getLoopLength()) {
                    canScroll = false;
                }
            }
            if (canScroll)
    ...
    private void handleSysEx(final String data) {
        ...
        // bytes[0] = F0, bytes[1-3] = manufacturer, bytes[4] = controller mode,
        // bytes[5] = command
        if (bytes.length < 7)
            return;
    ...
    private void handleSidebarPadEvent(final byte[] bytes) {
        ...
        // bytes[0]=F0, [1-3]=manufacturer, [4]=mode, [5]=cmd, [6]=sidebarCol, [7]=row,
        // [8]=state
